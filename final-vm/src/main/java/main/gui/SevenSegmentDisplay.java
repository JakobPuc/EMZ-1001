...
public class SevenSegmentDisplay extends Group {
	...
	public SevenSegmentDisplay(double x, double y, double scale) {
		...
		// Create segments with proper lengths
		this.segments[0] = createSegment(x + thickness, y, hLen, thickness); // A - top
		this.segments[1] = createSegment(x + hLen + thickness, y + thickness, thickness, vLen); // B - top right
		this.segments[2] = createSegment(x + hLen + thickness, y + vLen + 2 * thickness, thickness,
				vLen - thickness); // C - bottom right (shortened)
		this.segments[3] = createSegment(x + thickness, y + 2 * vLen + thickness, hLen, thickness); // D -
														// bottom
														// (moved
														// up
														// slightly)
		this.segments[4] = createSegment(x, y + vLen + 2 * thickness, thickness, vLen - thickness); // E -
														// bottom
														// left
														// (shortened)
		this.segments[5] = createSegment(x, y + thickness, thickness, vLen); // F - top left
		this.segments[6] = createSegment(x + thickness, y + vLen + thickness, hLen, thickness); // G - middle
